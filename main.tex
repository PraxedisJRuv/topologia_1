...
\\Supongamos que $X-\overline{A}=\overline{X-A}$. Sabemos que $X-\overline{A}=(\overline{A})^c$ es abierto, además de que $\overline{X-A}=\overline{A^c}$ es cerrado. Entonces $((\overline{A})^c)^c)=\overline{A}$, lo cual es cerrado, pero también $\overline{A}=(\overline{A^c})^c$, donde esto último sería abierto. Por lo que es una contradicción tanto al decir que la cerradura es abierta como que el conjunto previo que se determinó cerrado, por la transitivdad de la igualdad sería también abierto.
